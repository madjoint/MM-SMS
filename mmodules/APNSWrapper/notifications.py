...
class APNSProperty(object):
    ...
    def _build(self):
        ...
        if isinstance(self.data, (tuple, list)):
            arguments = [if_else(isinstance(x, str), '"%s"' % _doublequote(str(x)), str(x)) for x in self.data]
...
class APNSNotification(object):
    ...
    def alert(self, alert = None):
        ...
        """
        if not isinstance(alert, str) and not isinstance(alert, str) and not isinstance(alert, APNSAlert):
